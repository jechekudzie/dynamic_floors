    ...
    <div class="content-body">
        ...
        <div class="add-edit-product-wrap col-12">
            ...
            <div class="add-edit-product-form">
                <form action="{{url('/admin/cleaners/'.$cleaner->id)}}" method="post"
                      enctype="multipart/form-data">
                    @method('PATCH')
                    @csrf
                    <h4 class="title">Edit {{$cleaner->name}}</h4>

                    <div class="row">

                        <div class="col-lg-6 col-12 mb-30"><input name="name" value="{{$cleaner->name}}"
                                                                  class="form-control" type="text"
                                                                  placeholder="Cleaner name*"></div>
                        <div class="col-12 mb-30"><textarea name="description" class="summernote form-control"
                                                            placeholder="Product
                        Description*">{{$cleaner->description}}</textarea></div>

                    </div>
                    <h4 class="current">Current cover image </h4>
                    <img style="width: 100px;" class="img-100" src="{{asset($cleaner->image)}}">
